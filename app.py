...
def refs(question):
    q = norm(question)
    p = r"\bit\s*(?:(?:n|no)\s*)?0?(\d{1,2})\s*[-/ ]\s*(20\d{2}|\d{2})\b"
    return [(int(n), int(a) if len(a)==4 else 2000+int(a)) for n,a in re.findall(p,q)]


def grupos(question):
    return set(re.findall(r"\b[A-Z]{1,3}\s*[-–—]?\s*\d{1,3}\b", str(question).upper()))


def grupo_normalizado(codigo):
    return re.sub(r"[^A-Z0-9]", "", codigo.upper())


def grupos_na_pagina(texto):
    xs = re.findall(r"\b[A-Z]{1,3}\s*[-–—]?\s*\d{1,3}\b", str(texto).upper())
    return {grupo_normalizado(x) for x in xs}


def eh_it(nome, numero, ano=2025):
    n = norm(nome)
    return bool(re.search(rf"\bit\s*(?:(?:n|no)\s*)?0?{numero}\s*(?:-|/|\s)*({ano}|25)\b", n, re.I))
...
def recuperar(question,docs):
    ...
    for x in candidatos:
        ...
        vistos.add(k); x={**x,"texto":trecho_relevante(x["texto"],question)}; escolhidos.append(x)
