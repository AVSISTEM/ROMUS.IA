...
# Modelo oficial estável e de baixa latência para extração/documentos.
...
def assinatura_base() -> tuple:
    """Lê somente metadados. Não abre nenhum PDF."""
    ...
    for caminho in sorted(KNOWLEDGE_DIR.rglob("*")):
        if caminho.is_file() and caminho.suffix.lower() in {".txt", ".md", ".pdf"}:
            try:
                ...
                itens.append(
                    (
                        caminho.relative_to(KNOWLEDGE_DIR).as_posix(),
                        stat.st_size,
                        stat.st_mtime_ns,
                    )
                )
...
def listar_arquivos(signature: tuple) -> list[dict]:
    """Cria o catálogo sem ler o conteúdo dos documentos."""
    del signature
    documentos = []
    for caminho in sorted(KNOWLEDGE_DIR.rglob("*")):
...
def ler_documento(arquivo: str, tamanho: int, mtime_ns: int) -> dict | None:
    """Abre somente o documento escolhido pela busca."""
...
def extrair_referencias(pergunta: str) -> dict:
    ...
    for numero, ano in re.findall(
        r"\bit\s*(?:n[ºo°]?\s*)?(\d{1,2})\s*[-/]\s*(20\d{2})\b", q
    ):
        par = (int(numero), int(ano))
        if par not in referencias["it"]:
            referencias["it"].append(par)

    # Aceita também "IT 11/25" ou "IT nº 11-25".
    for numero in re.findall(r"\bit\s*(?:n[ºo°]?\s*)?(\d{1,2})\s*[-/]\s*25\b", q):
        par = (int(numero), 2025)
        if par not in referencias["it"]:
            referencias["it"].append(par)

    referencias["item"] = re.findall(
        r"\b(?:item|subitem|alínea|alinea)\s*(\d+(?:\.\d+){1,4})\b", q
    )
    referencias["decreto"] = re.findall(
...
def selecionar_arquivos(pergunta: str, catalogo: list[dict]) -> list[dict]:
    """Escolhe arquivos antes de abrir PDFs."""
    ...
    # Referência explícita: somente o documento correspondente.
    if refs["it"]:
    ...
    # Fallback excepcional: evita carregar a base inteira nas consultas normativas usuais.
    return avaliados[:2]
...
def carregar_selecionados(candidatos: list[dict]) -> list[dict]:
    ...
    for candidato in candidatos:
        doc = ler_documento(
            candidato["arquivo"], candidato["tamanho"], candidato["mtime_ns"]
        )
        if not doc:
            continue
...
def buscar_base(pergunta: str, documentos: list[dict]) -> list[dict]:
    ...
    for doc in documentos:
        doc_score = pontuar_nome(
            pergunta,
            {"nome": doc["nome"], "nome_norm": doc["nome_norm"]},
        )
    ...
    for item in resultados:
        chave = hashlib.sha1(
            re.sub(r"\s+", " ", item["texto"]).strip().lower().encode(
                "utf-8", errors="ignore"
            )
        ).hexdigest()
        if chave in vistos:
...
def pedido_literal(pergunta: str) -> bool:
    ...
    return bool(
        termos.intersection(
            {"transcreva", "transcrever", "literalmente", "literal", "exatamente", "redacao"}
        )
    )
...
def pergunta_pede_calculo(pergunta: str) -> bool:
    ...
    return bool(
        termos.intersection(
            {"calcule", "calcular", "calculo", "dimensione", "dimensionar"}
        )
    )
...
def extrair_numero_item(pergunta: str) -> str | None:
    m = re.search(
        r"\b(?:item|subitem)\s*(\d+(?:\.\d+){1,4})\b", pergunta.lower()
    )
...
def melhor_linha_explicita(pergunta: str, resultados: list[dict]) -> str | None:
    ...
    if intencao == "decreto":
        ...
        prioritarias = {
            "saidas", "emergencia", "largura", "unidade",
            "passagem", "porta", "minima", "minimo"
        }
    else:
        prioritarias = termos
    ...
    for item in resultados:
        for linha in re.split(r"\n+", item["texto"]):
            ...
            if intencao == "decreto" and "decreto" in lt:
                score += 80
            if refs["it"] and (
                "largura" in lt or "saida" in lt or "emergencia" in lt
            ):
                score += 35
...
def resposta_local_deterministica(pergunta: str, resultados: list[dict]) -> str | None:
    ...
    # Perguntas simples devem ser respondidas sem Gemini.
...
web_habilitada = st.checkbox(
    "Pesquisar na web se a base não responder", value=True
)
...
if perguntar and pergunta.strip():
    ...
    resposta_direta = resposta_local_deterministica(
        pergunta.strip(), resultados
    )
    if resposta_direta:
    ...
    # CAMADA 2 — Gemini somente para síntese/cálculo.
    client = obter_client()

    if resultados and client is not None:
        with st.spinner("Sintetizando a resposta com base no documento..."):
            try:
                resposta = gerar_resposta_local(
                    client, pergunta.strip(), resultados
                )
        ...
        # Se a base encontrou conteúdo, mas Gemini falhou,
        # NÃO pula para a internet. A regra é: web só quando a base for insuficiente.
        if texto.startswith("ERRO_GEMINI:"):
            st.markdown("### ROMUS.IA")
            st.warning(
                "A base local encontrou conteúdo relevante, mas a síntese pelo Gemini "
                "não está disponível no momento."
            )
            mostrar_fontes(resultados)
            st.stop()

    # CAMADA 3 — internet somente se a base não tiver resposta.
